...
class IndexController extends Controller {
	 ...
	 * 数据库文件
	 */
	protected $sql_file = './MenggeCMS/Install/Data/thinkcms.sql';
	/**
	 * 配置模版文件
	 */
	protected $conf_tmpl = './MenggeCMS/Install/Data/config.php';
	/**
	 * 系统配置文件位置
	 */
	protected $conf_file = './MenggeCMS/Common/Conf/config.php';
	/**
	 * 安装步骤名称
	 */
    ...
    public function s4(){
    	if(intval($_GET['install'])){
    		...
    		$dbPrefix = empty($_POST['dbprefix']) ? 'glb_' : trim($_POST['dbprefix']);
			...
			$query = "INSERT INTO `{$dbPrefix}admin` (username,password,email,nickname,status,addtime,lastlogin,lastlogintime) VALUES ('{$username}', '{$password}', '{$email}', '', '1', '{$create_date}','$ip','$create_date');";
			mysql_query($query);
